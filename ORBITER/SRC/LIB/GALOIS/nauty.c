      ...
      set *ws_arg, int worksize, int m_arg, int n_arg, graph *canong_arg)
{
	...
	ago = 1;
#if !MAXN
	/*printf("nauty before firstpathnode0\n");*/
        retval = firstpathnode0(lab,ptn,1,numcells,&tcnode0);
#else
	/*printf("nauty before firstpathnode\n");*/
...
#endif
{
    ...
    /* refine partition : */
	/*printf("nauty firstpathnode0/firstpathnode before doref\n");*/
	...
	/*printf("nauty firstpathnode0/firstpathnode after doref numcells=%d\n", numcells);*/
    ...
    /* use the elements of the target cell to produce the children: */
                                        ...
                                        tv = nextelement(tcell,M,tv))
        {
            if (orbits[tv] == tv)   /* ie, not equiv to previous child */
            {
				...
				if (tv == tv1)
                {
			/*printf("nauty firstpathnode before firstpathnode\n");*/
#if !MAXN
...
#endif
			/*printf("nauty firstpathnode after firstpathnode\n");*/
                    childcount = 1;
                    gca_first = level;
                    stabvertex = tv1;
				}
                ...
                {
			/*printf("nauty firstpathnode before othernode\n");*/
#if !MAXN
...
#endif
			/*printf("nauty firstpathnode after othernode\n");*/
                    ++childcount;
                }
...
processnode(int *lab, int *ptn, int level, int numcells)
{
	...
	/*printf("nauty processnode\n");*/
        ...
        case 2:                 /* lab is equivalent to canonlab */
            ...
            if (writeautoms)
                writeperm(outfile,workperm,cartesian,linelength,n);
			/*printf("nauty before store_perm case 2\n");*/
			store_perm(workperm, n);  // Abdullah !!!
			++stats->numgenerators;
			aut_counter = stats->numgenerators;
			OPTCALL(userautomproc)(stats->numgenerators,workperm,orbits,
