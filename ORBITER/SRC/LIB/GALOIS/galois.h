...
typedef class homogeneous_polynomial_domain homogeneous_polynomial_domain; // added Sept 9, 2016
...
public:
	...
	void matrix_minor(INT f_semilinear, INT *A, INT *B, INT n, INT f, INT l);
		// initializes B as the l x l minor of A (which is n x n) starting from row f. 
	...
	void semilinear_matrix_mult_memory_given(INT *A, INT *B, INT *AB, INT *tmp_B, INT n);
		// (A,f1) * (B,f2) = (A*B^{\varphi^{-f1}},f1+f2)
	...
	void projective_action_from_the_right(INT f_semilinear, INT *v, INT *A, INT *vA, INT n, INT verbose_level);
		// vA = (v * A)^{p^f}  if f_semilinear (where f = A[n *  n]),   vA = v * A otherwise
	...
	INT is_contained_in_subspace(INT k, INT len, INT *basis, INT *base_cols, 
		INT *v, INT verbose_level);
	void compute_and_print_projective_weights(INT *M, INT n, INT k);
	INT code_minimum_distance(INT n, INT k, INT *code, INT verbose_level);
		// code[k * n]
...
public:
	...
	void print_types_naked_tex(ostream &ost, INT f_backwards, INT *the_vec_sorted, 
	...
	set_of_sets *get_set_partition_and_types(INT *&types, INT &nb_types, INT verbose_level);
};
...
public:
	...
	mp_graphics();
	mp_graphics(const char *file_name, INT xmin, INT ymin, INT xmax, INT ymax, 
		INT f_embedded, INT f_sideways);
	~mp_graphics();
	void default_values();
	void init(const char *file_name, INT xmin, INT ymin, INT xmax, INT ymax, INT f_embedded, INT f_sideways);
	...
	void polygon_or_bezier_idx_mp(INT *Px, INT *Py, INT *Idx, INT n, const char *symbol, INT f_cycle);
	...
	void polygon_or_bezier_idx_tikz(INT *Px, INT *Py, INT *Idx, INT n, const char *symbol, INT f_cycle);
...
class incidence_structure {
	...
	void get_and_print_row_tactical_decomposition_scheme_tex(
		ostream &ost, INT f_enter_math, INT f_print_subscripts, partitionstack &PStack);
...
public:
	...
	void perp(INT pt, INT *Perp_without_pt, INT &sz, INT verbose_level);
	void perp_of_two_points(INT pt1, INT pt2, INT *Perp, INT &sz, INT verbose_level);
	void perp_of_k_points(INT *pts, INT nb_pts, INT *&Perp, INT &sz, INT verbose_level);
};
...
public:
	...
	INT *embedding; // [big_n - n], the columns which are not base_cols in increasing order
	INT *Tmp1; // [big_n]
	INT *Tmp2; // [big_n]
	INT *Tmp3; // [big_n]
	INT *tmp_M1; // [n * n]
	...
	void init(INT big_n, INT n, grassmann *G, INT *M, INT verbose_level);
		// M is n x big_n
	void unrank_embedded_INT(INT *subspace_basis_with_embedding, INT rk, INT verbose_level);
		// subspace_basis_with_embedding is n x big_n
	INT rank_embedded_INT(INT *subspace_basis, INT verbose_level);
		// subspace_basis is n x big_n, only the first k x big_n entries are used
...
public:
	...
	void null();
	void freeself();
	void init(INT n, INT *type, INT type_len, finite_field *F, INT verbose_level);
	void init_recursion(INT n, INT *type, INT type_len, INT idx, finite_field *F, INT verbose_level);
...
void projective_plane_draw_grid(const char *fname, INT xmax, INT ymax, INT f_with_points, INT rad, 
	...
	INT f_embedded, INT f_sideways, 
	INT verbose_level);
void projective_plane_draw_grid2(mp_graphics &G, INT q, INT *Table, INT nb, INT f_with_points, INT rad, 
...
public:
	...
	void init_from_file(INT underlying_set_size, const BYTE *fname, INT verbose_level);
	void init_from_csv_file(INT underlying_set_size, const BYTE *fname, INT verbose_level);
	void init_from_orbiter_file(INT underlying_set_size, const BYTE *fname, INT verbose_level);
...
public:
	...
	void fill_entry_with_text(INT row_idx, INT col_idx, const BYTE *text);
	void fill_column_with_text(INT col_idx, const BYTE **text, const BYTE *heading);
	void fill_column_with_INT(INT col_idx, INT *data, const BYTE *heading);
	void fill_column_with_row_index(INT col_idx, const BYTE *heading);
	...
	void print_table_latex_all_columns(ostream &ost, INT f_enclose_in_parentheses);
	void print_table_latex(ostream &ost, INT *f_column_select, INT f_enclose_in_parentheses);
	void print_table_row(INT row, INT f_enclose_in_parentheses, ostream &ost);
	void print_table_row_latex(INT row, INT *f_column_select, INT f_enclose_in_parentheses, ostream &ost);
...
public:
	...
	void create_points_on_line(INT line_rk, INT *line, INT verbose_level);
		// needs line[k]
	...
	INT determine_line_in_plane(INT *two_input_pts, INT *three_coeffs, INT verbose_level);
	...
	void compute_bisecants_and_conics(INT *arc6, INT *&bisecants, INT *&conics, INT verbose_level);
	// bisecants[15 * 3]
	// conics[6 * 6]
	void find_Eckardt_points_from_arc_not_on_conic(INT *arc6, eckardt_point *&E, INT &nb_E, INT verbose_level);
	void find_Eckardt_points_from_arc_not_on_conic_prepare_data(INT *arc6, 
		INT *&bisecants, // [15]
	...
	void line_intersection_type_basic(INT *set, INT set_size, INT *type, INT verbose_level);
		// type[N_lines]
	void line_intersection_type_through_hyperplane(INT *set, INT set_size, INT *type, INT verbose_level);
	// type[N_lines]
	void find_secant_lines(INT *set, INT set_size, INT *lines, INT &nb_lines, INT max_lines, INT verbose_level);
	void find_lines_which_are_contained(INT *set, INT set_size, INT *lines, INT &nb_lines, INT max_lines, INT verbose_level);
	...
	void points_on_projective_triangle(INT *&set, INT &set_size, INT *three_points, INT verbose_level);
	...
	void line_plane_incidence_matrix_restricted(INT *Lines, INT nb_lines, INT *&M, INT &nb_planes, INT verbose_level);
	INT test_if_lines_are_skew(INT line1, INT line2, INT verbose_level);
	INT point_of_intersection_of_a_line_and_a_line_in_three_space(INT line1, INT line2, INT verbose_level);
	INT point_of_intersection_of_a_line_and_a_plane_in_three_space(INT line, INT plane, INT verbose_level);
	INT line_of_intersection_of_two_planes_in_three_space(INT plane1, INT plane2, INT verbose_level);
	INT line_of_intersection_of_two_planes_in_three_space_using_dual_coordinates(INT plane1, INT plane2, INT verbose_level);
	void plane_intersection_matrix_in_three_space(INT *Planes, INT nb_planes, INT *&Intersection_matrix, INT verbose_level);
	INT dual_rank_of_plane_in_three_space(INT plane_rank, INT verbose_level);
	void plane_equation_from_three_lines_in_three_space(INT *three_lines, INT *plane_eqn4, INT verbose_level);
...
void do_draw_points_in_plane(finite_field *F, 
	INT *set, INT set_size, 
	const BYTE *fname_base, INT f_point_labels, INT f_embedded, INT f_sideways, INT verbose_level);
...
public:
	...
	INT solve_all_DLX_with_RHS_and_callback(INT f_write_tree, const BYTE *fname_tree, 
	...
	void solve_mckay(const BYTE *label, INT maxresults, INT &nb_backtrack_nodes, INT &nb_sol, INT verbose_level);
	...
	void draw_partitioned(const BYTE *fname_base, INT xmax_in, INT ymax_in, INT xmax_out, INT ymax_out, 
...
public:
	...
	INT *pt_list; // [max_pts] list of points, used only in compute_system_in_RREF
	...
	INT *Trihedral_to_Eckardt; // [nb_trihedral_pairs * 6] first the three rows, then the three columns

	INT nb_collinear_Eckardt_triples; // nb_trihedral_pairs * 2
	INT *collinear_Eckardt_triples_rank; // as three subsets of 45 = nb_Eckardt_points

	classify *Classify_collinear_Eckardt_triples;

	homogeneous_polynomial_domain *Poly1; // linear polynomials in three variables
	homogeneous_polynomial_domain *Poly2; // quadratic polynomials in three variables
	homogeneous_polynomial_domain *Poly3; // cubic polynomials in three variables

	homogeneous_polynomial_domain *Poly1_x123; // linear polynomials in three variables
	homogeneous_polynomial_domain *Poly2_x123; // quadratic polynomials in three variables
	homogeneous_polynomial_domain *Poly3_x123; // cubic polynomials in three variables
	homogeneous_polynomial_domain *Poly4_x123; // quartic polynomials in three variables

	homogeneous_polynomial_domain *Poly1_4; // linear polynomials in four variables
	homogeneous_polynomial_domain *Poly2_4; // quadratic polynomials in four variables
	homogeneous_polynomial_domain *Poly3_4; // cubic polynomials in four variables
	...
	INT *Half_double_sixes; // [72 * 6] 
		...
		// so the pairing between the lines in the associated double six is gone.
	...
	void label_variables_3(homogeneous_polynomial_domain *HPD, INT verbose_level);
	void label_variables_x123(homogeneous_polynomial_domain *HPD, INT verbose_level);
	void label_variables_4(homogeneous_polynomial_domain *HPD, INT verbose_level);
	void label_variables_27(homogeneous_polynomial_domain *HPD, INT verbose_level);
	void label_variables_24(homogeneous_polynomial_domain *HPD, INT verbose_level);
	...
	void compute_intersection_points(INT *Adj, 
		...
		INT *&Intersection_pt,  
		INT verbose_level);
	void compute_intersection_points_and_indices(INT *Adj, INT *Points, INT nb_points, 
		INT *Lines, INT nb_lines, 
		INT *&Intersection_pt, INT *&Intersection_pt_idx, 
		INT verbose_level);
	INT create_double_six_from_six_disjoint_lines(INT *single_six, INT *double_six, INT verbose_level);
	void latex_double_six(ostream &ost, INT *double_six);
	void create_the_fifteen_other_lines(INT *double_six, INT *fifteen_other_lines, INT verbose_level);
	void compute_adjacency_matrix_of_line_intersection_graph(INT *&Adj, INT *S, INT n, INT verbose_level);
	void compute_adjacency_matrix_of_line_disjointness_graph(INT *&Adj, INT *S, INT n, INT verbose_level);
	void compute_points_on_lines(INT *Pts_on_surface, INT nb_points_on_surface, 
		INT *Lines, INT nb_lines, 
		set_of_sets *&pts_on_lines, 
		INT verbose_level);
	void lines_meet3_and_skew3(INT *lines_meet3, INT *lines_skew3, INT *&lines, INT &nb_lines, INT verbose_level);
	void perp_of_three_lines(INT *three_lines, INT *&perp, INT &perp_sz, INT verbose_level);
	INT perp_of_four_lines(INT *four_lines, INT *trans12, INT &perp_sz, INT verbose_level);
	INT rank_of_four_lines_on_Klein_quadric(INT *four_lines, INT verbose_level);
	INT create_double_six_from_five_lines_with_a_common_transversal(INT *five_pts, INT *double_six, INT verbose_level);
	INT test_special_form_alpha_beta(INT *coeff, INT &alpha, INT &beta, INT verbose_level);
	void create_special_double_six(INT *double_six, INT a, INT b, INT verbose_level);
	void create_special_fifteen_lines(INT *fifteen_lines, INT a, INT b, INT verbose_level);
	void create_remaining_fifteen_lines(INT *double_six, INT *fifteen_lines, INT verbose_level);
	INT compute_cij(INT *double_six, INT i, INT j, INT verbose_level);
	INT compute_transversals_of_any_four(INT *&Trans, INT &nb_subsets, INT *lines, INT sz, INT verbose_level);
	INT compute_rank_of_any_four(INT *&Rk, INT &nb_subsets, INT *lines, INT sz, INT verbose_level);
	...
	void make_trihedral_pairs(INT *&T, BYTE **&T_label, INT &nb_trihedral_pairs, INT verbose_level);
	...
	void find_trihedral_pairs_from_collinear_triples_of_Eckardt_points(INT *E_idx, INT nb_E, 
	...
	void create_lines_from_plane_equations(INT *The_plane_equations, INT *Lines, INT verbose_level);
	void create_web_of_cubic_curves_and_equations_based_on_four_tritangent_planes(INT *arc6, INT *base_curves4, 
	...
	void create_lambda_from_trihedral_pair_and_arc(INT *arc6, INT *Web_of_cubic_curves, 
		INT *The_plane_equations, INT t_idx, INT &lambda, INT &lambda_rk, INT verbose_level);
	void create_surface_equation_from_trihedral_pair(INT *arc6, INT *Web_of_cubic_curves, 
		INT *The_plane_equations, INT t_idx, INT *surface_equation, INT &lambda, INT verbose_level);
#if 0
	void create_surface_equation_from_trihedral_pair2(INT *arc6, INT *Web_of_cubic_curves, 
		INT *The_plane_equations, INT t_idx0, INT t_idx, INT *eqn, INT &lambda, INT verbose_level);
	void create_surface_and_planes_from_trihedral_pair_and_arc(INT *arc6, INT t_idx, 
		INT *planes, INT *the_equations, INT &lambda, INT verbose_level);
#endif
	void extract_six_curves_from_web(INT *Web_of_cubic_curves, 
		INT *row_col_Eckardt_points, INT *six_curves, INT verbose_level);
	void find_point_not_on_six_curves(INT *arc6, INT *six_curves, INT &pt, INT &f_point_was_found, INT verbose_level);
	INT plane_from_three_lines(INT *three_lines, INT verbose_level);
	void Trihedral_pairs_to_planes(INT *Lines, INT *Planes, INT verbose_level);
		// Planes[nb_trihedral_pairs * 6]
	void rearrange_lines_according_to_double_six(INT *Lines, INT verbose_level);
#if 0
	void find_matching_trihedral_pair(INT *Lines, INT *nine_lines, INT &t_idx, INT verbose_level);
	void rearrange_lines_according_to_trihedral_pair(INT *Lines, 
		INT t_idx, INT *nine_lines, INT verbose_level);
	INT test_permutation_equivalence_for_nine_lines(INT *nine_lines_a, INT *nine_lines_b, INT verbose_level);
#endif
	void rearrange_lines_according_to_starter_configuration(INT *Lines, INT *New_lines, 
	...
	void create_surface_family_S(INT a, INT *Lines27, INT *equation20, INT verbose_level);
#if 0
	void create_surface(INT f_iso, INT iso, INT f_S, INT a, INT f_equation, const BYTE *eqn_txt, 
	...
	void compute_tritangent_planes(INT *Lines, 
		...
		INT verbose_level);
	void compute_external_lines_on_three_tritangent_planes(INT *Lines, INT *&External_lines, INT &nb_external_lines, INT verbose_level);
	...
	INT clebsch_map(INT *Lines, INT *Pts, INT nb_pts, INT line_idx[2], INT plane_rk, 
	...
	void compute_nine_lines(INT *F_planes, INT *G_planes, INT *nine_lines, INT verbose_level);
	void compute_nine_lines_by_dual_point_ranks(INT *F_planes_rank, INT *G_planes_rank, INT *nine_lines, INT verbose_level);
	void print_trihedral_pair_in_dual_coordinates_in_GAP(INT *F_planes_rank, INT *G_planes_rank);
...
public:
	...
	INT *Variables; // [nb_monomials * degree]
		...
		// as a sequence of length degree with entries in 0,..,n-1.
		// the entries are listed in increasing order.
		// For instance, the monomial x_0^2x_1x_3 is recorded as 0,0,1,3
	...
	void freeself();
	void null();
	void init(finite_field *F, INT nb_vars, INT degree, INT f_init_incidence_structure, INT verbose_level);
	...
	void print_equation(ostream &ost, INT *coeffs);
	void print_equation_with_line_breaks_tex(ostream &ost, INT *coeffs, INT nb_terms_per_line, const BYTE *new_line_text);
	void enumerate_points(INT *coeff, INT *Pts, INT &nb_pts, INT verbose_level);
	...
	void vanishing_ideal(INT *Pts, INT nb_pts, INT &r, INT *Kernel, INT verbose_level);
...
public:
	...
	set_of_sets *pts_on_lines; // points are stored as indices into Pts[]
	...
	void print_trihedral_pairs(ostream &ost);
	void latex_table_of_trihedral_pairs_and_clebsch_system(ostream &ost, INT *T, INT nb_T);
	...
	void identify_double_six_from_trihedral_pair(INT *Lines, INT t_idx, INT *nine_lines, INT *double_sixes, INT verbose_level);
	void identify_double_six_from_trihedral_pair_type_one(INT *Lines, INT t_idx, INT *nine_line_idx, INT *double_sixes, INT verbose_level);
	void identify_double_six_from_trihedral_pair_type_two(INT *Lines, INT t_idx, INT *nine_line_idx, INT *double_sixes, INT verbose_level);
	void identify_double_six_from_trihedral_pair_type_three(INT *Lines, INT t_idx, INT *nine_line_idx, INT *double_sixes, INT verbose_level);
	void find_common_transversals_to_two_disjoint_lines(INT a, INT b, INT *transversals5);
	void find_common_transversals_to_three_disjoint_lines(INT a1, INT a2, INT a3, INT *transversals3);
	void find_common_transversals_to_four_disjoint_lines(INT a1, INT a2, INT a3, INT a4, INT *transversals2);
	INT find_tritangent_plane_through_two_lines(INT line_a, INT line_b);
	void get_planes_through_line(INT *new_lines, INT line_idx, INT *planes5);
	void find_two_lines_in_plane(INT plane_idx, INT forbidden_line, INT &line1, INT &line2);
	INT find_unique_line_in_plane(INT plane_idx, INT forbidden_line1, INT forbidden_line2);
	void identify_lines(INT *lines, INT nb_lines, INT *line_idx, INT verbose_level);
	void print_nine_lines_latex(ostream &ost, INT *nine_lines, INT *nine_lines_idx);
	INT choose_tritangent_plane(INT line_a, INT line_b, INT transversal_line, INT verbose_level);
...
public:
	...
	INT *Type_rep;
		...
		// has been chosen as representative for the i-th isomorphism type
	INT *Type_mult; 
		// Type_mult[N]
	...
	INT *perm;
		// the permutation which lists the orbit representative in the order 
...
public:
	...
	object_in_projective_space_type type;
		...
		// t_PAC = a packing (i.e. q^2+q+1 sets if lines of size q^2+1)
	...
	void init_point_set(projective_space *P, INT *set, INT sz, INT verbose_level);
	void init_line_set(projective_space *P, INT *set, INT sz, INT verbose_level);
	void init_packing_from_set(projective_space *P, INT *packing, INT sz, INT verbose_level);
	void init_packing_from_set_of_sets(projective_space *P, set_of_sets *SoS, INT verbose_level);
	...
	void encode_incma(INT *&Incma, INT &nb_rows, INT &nb_cols, INT *&partition, INT verbose_level);
	void encode_point_set(INT *&Incma, INT &nb_rows, INT &nb_cols, INT *&partition, INT verbose_level);
	void encode_line_set(INT *&Incma, INT &nb_rows, INT &nb_cols, INT *&partition, INT verbose_level);
	void encode_packing(INT *&Incma, INT &nb_rows, INT &nb_cols, INT *&partition, INT verbose_level);
	void encode_incma_and_make_decomposition(
		INT *&Incma, INT &nb_rows, INT &nb_cols, INT *&partition, 
		incidence_structure *&Inc, 
		partitionstack *&Stack, 
		INT verbose_level);
	void encode_object(INT *&encoding, INT &encoding_sz, INT verbose_level);
	void encode_object_points(INT *&encoding, INT &encoding_sz, INT verbose_level);
	void encode_object_lines(INT *&encoding, INT &encoding_sz, INT verbose_level);
