...
void orbit_transversal::read_from_file(action *A, action *A2, const BYTE *fname, INT verbose_level)
{
	...
	if (f_v) {
		cout << "orbit_transversal::read_from_file before read_and_parse_data_file_fancy" << endl;
		}
	...
	if (f_v) {
		cout << "orbit_transversal::read_from_file processing " << nb_orbits << " orbit representatives" << endl;
		}


	Reps = new set_and_stabilizer[nb_orbits];
	...
	for (i = 0; i < nb_cases; i++) {
		...
		if (f_v && ((i + 1) % nb_cases_mod) == 0) {
			cout << "orbit_transversal::read_from_file processing case " << i << " / " << nb_orbits << " : " << 100. * (double) i / (double) nb_cases << "%" << endl;
			}
		strong_generators *gens;
		INT *set;

		gens = new strong_generators;
		gens->init_from_ascii_coding(A, Aut_ascii[i], 0 /* verbose_level */);
