	...
	INT verbose_level)
{
	...
	magma_normalizer_in_Sym_n(fname_base, group_order, Table, gens, nb_gens, 
	...
	A->init_permutation_group_from_generators(group_order /* degree */, 
		TRUE, go, 
		N_nb_gens, N_gens, 
	...
	INT verbose_level)
{
	...
	if (f_v) {
		cout << "action_global.C create_linear_group m=" << m << " q=" << F->q << endl;
		}
	...
	if (f_projective) {
		if (f_v) {
			cout << "action_global.C before init_projective_group m=" << m << " q=" << F->q << " f_semilinear=" << f_semilinear << endl;
			}
		...
		if (f_v) {
			cout << "action_global.C before init_general_linear_group m=" << m << " q=" << F->q << " f_semilinear=" << f_semilinear << endl;
			}
		...
		if (f_v) {
			cout << "action_global.C before init_affine_group m=" << m << " q=" << F->q << " f_semilinear=" << f_semilinear << endl;
			}
		A->init_affine_group(m, F, f_semilinear, 
			f_basis, verbose_level);
		}
	else {
		cout << "action_global.C create_linear_group the type of group is not specified" << endl;
		exit(1);
		}


	if (!A->f_has_strong_generators) {
		cout << "action_global.C create_linear_group fatal: !A->f_has_strong_generators" << endl;
		}
	...
	if (f_special) {
		...
		if (f_v) {
			cout << "action_global.C create_linear_group computing intersection with special linear group" << endl;
			}
		...
		if (f_v) {
			cout << "action_global.C create_linear_group induced_action_on_determinant finished" << endl;
			}
		A_on_det.Kernel->group_order(go);
		if (f_v) {
			cout << "action_global.C create_linear_group intersection has order " << go << endl;
			}
		...
		if (f_v) {
			cout << "action_global.C creating strong generators for the kernel n the action on the determinant" << endl;
			}
	...
	INT verbose_level)
{
	...
	if (f_v) {
		cout << "generators_to_strong_generators before create_sims_from_generators_randomized" << endl;
		}
	...
	if (f_v) {
		cout << "generators_to_strong_generators after create_sims_from_generators_randomized" << endl;
		}
...
action *create_automorphism_group_of_graph(INT *Adj, INT n, INT verbose_level)
{
	...
	if (f_v) {
		cout << "create_automorphism_group_of_graph before nauty_interface_graph_INT" << endl;
		}
	nauty_interface_graph_INT(n, Adj, 
		labeling, partition, 
		Aut, Aut_counter, 
...
// labeling[n]
{
	...
	if (f_v) {
		cout << "create_automorphism_group_and_canonical_labeling_of_graph n=" << n << endl;
		}
	...
	if (f_v) {
		cout << "create_automorphism_group_and_canonical_labeling_of_graph" << endl;
		}
	for (i = 0; i < n; i++) {
		partition[i] = 1;
		}
	...
	if (f_v) {
		cout << "create_automorphism_group_and_canonical_labeling_of_graph before nauty_interface_graph_INT" << endl;
		}
	nauty_interface_graph_INT(n, Adj, 
		labeling, partition, 
		Aut, Aut_counter, 
		...
		Transversal_length, Ago, verbose_level);
	if (f_v) {
		cout << "create_automorphism_group_and_canonical_labeling_of_graph after nauty_interface_graph_INT" << endl;
		}
	action *A;
	longinteger_object ago;
	...
	if (f_v) {
		cout << "create_automorphism_group_and_canonical_labeling_of_graph: created action ";
		A->print_info();
		cout << endl;
		}
	...
	//FREE_INT(labeling);
	if (f_v) {
		cout << "create_automorphism_group_and_canonical_labeling_of_graph done" << endl;
		}
	return A;
}
...
// December 31, 2014, moved here from projective_space.C
{
	...
	if (f_v) {
		cout << "set_stabilizer_in_projective_space before init_permutation_group_from_generators" << endl;
		}
	ago.create(Ago);
	...
	for (g = 0; g < gens1->len; g++) {
		...
		for (i = 0; i < P->N_points; i++) {
			...
			if (j1 != j2) {
				cout << "set_stabilizer_in_projective_space problem with generator: j1 != j2" << endl;
				cout << "i=" << i << endl;
				cout << "j1=" << j1 << endl;
				cout << "j2=" << j2 << endl;
				cout << endl;
	...
	if (D.compare_unsigned(ago, go)) {
		cout << "set_stabilizer_in_projective_space: the group order does not match" << endl;
		cout << "ago = " << ago << endl;
		cout << "go = " << go << endl;
		exit(1);
		}
	...
	const BYTE *fname_base, INT verbose_level)
{
	...
	if (f_v) {
		cout << "do_canonical_form before set_stabilizer_in_projective_space" << endl;
		}
	...
	action *&A_on_equations, schreier *&Orb, INT verbose_level)
{
	...
	A_on_equations->induced_action_on_homogeneous_polynomials_given_by_equations(A, 
		...
		0 /*verbose_level */);
	if (f_v) {
		cout << "The induced action on the equations has been created, degree = " << A_on_equations->degree << endl;
		}
	...
	Orb = gens->orbits_on_points_schreier(A_on_equations, 0 /* verbose_level + 10 */);
	//Orb = SaS->Strong_gens->orbits_on_points_schreier(A_on_equations, verbose_level + 10);

	if (f_v) {
		cout << "We found " << Orb->nb_orbits << " orbits on the equations:" << endl;
