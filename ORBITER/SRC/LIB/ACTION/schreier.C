...
void schreier::init_generators_by_hdl(INT nb_gen, INT *gen_hdl, INT verbose_level)
{
	...
	if (f_vv) {
		cout << "schreier::init_generators_by_hdl generators:" << endl;
		gens.print(cout);
		}
	if (f_v) {
...
INT schreier::get_image(INT i, INT gen_idx, INT verbose_level)
{
	...
	if (f_v) {
		cout << "schreier::get_image computing image of point " << i << " under generator " << gen_idx << endl;
		}
	if (images == NULL) {
	...
	if (f_v) {
		cout << "schreier::get_image image of point " << i << " under generator " << gen_idx << " is " << a << endl;
		}
	return a;
}
	...
	void (*print_point)(ostream &ost, INT pt, void *data), void *data)
{
	...
	for (i = 0; i < nb_orbits; i++) {
		...
		ost << "Orbit " << i << " / " << nb_orbits << " : of length " << orbit_len[i];
		ost << " is:" << endl;
		print_orbit(ost, i);
		ost << endl;
		ost << "Which is:" << endl;
		...
		//ost << endl;
		ost << "The stabilizer of the element " << orbit[orbit_first[i]] << " is:" << endl;
...
void schreier::print_and_list_orbits(ostream &ost)
{
	...
	ost << "i : orbit_first[i] : orbit_len[i]" << endl;
	for (i = 0; i < nb_orbits; i++) {
		ost << " Orbit " << i << " / " << nb_orbits << " : " << orbit_first[i] << " : " << orbit_len[i];
		ost << " : ";
		print_orbit(ost, i);
		ost << endl;
		}
...
void schreier::print_and_list_orbits_tex(ostream &ost)
{
	...
	ost << nb_orbits << " orbits:\\\\" << endl;
	ost << "orbits under a group with " << gens.len << " generators acting on a set of size " << A->degree << ":\\\\" << endl;
	//ost << "i : orbit_first[i] : orbit_len[i]" << endl;
	for (i = 0; i < nb_orbits; i++) {
...
void schreier::print_and_list_orbit_and_stabilizer_tex(INT i, action *default_action, 
...
void schreier::print_and_list_orbit_and_stabilizer_with_list_of_elements_tex(INT i, action *default_action, 
		strong_generators *gens, ostream &ost)
{
	...
	gens_stab = generators_for_stabilizer_of_orbit_rep(default_action, 
		full_group_order, i, 0 /*verbose_level */);
	...
	if (sz_subgroup < 20) {
		gens->list_of_elements_of_subgroup(gens_stab, 
			Subgroup_elements_by_index, sz_subgroup, 0 /* verbose_level */);

		INT_vec_heapsort(Subgroup_elements_by_index, sz_subgroup);

		ost << "The subgroup consists of the following " << sz_subgroup << " elements:" << endl;
		ost << "$$" << endl;
		INT_vec_print_as_matrix(ost, Subgroup_elements_by_index, sz_subgroup, 10 /* width */, TRUE /* f_tex */);
		ost << "$$" << endl;
...
void schreier::print_and_list_orbits_sorted_by_length_tex(ostream &ost)
{
	print_and_list_orbits_sorted_by_length(ost, TRUE);
}

void schreier::print_and_list_orbits_sorted_by_length(ostream &ost)
{
	print_and_list_orbits_sorted_by_length(ost, FALSE);
}

void schreier::print_and_list_orbits_sorted_by_length(ostream &ost, INT f_tex)
{
	INT i, h;
	INT *Len;
	INT *Perm;
	...
	INT_vec_sorting_permutation(Len, nb_orbits, Perm, Perm_inv, TRUE /*f_increasingly*/);
	
	ost << "There are " << nb_orbits << " orbits under a group with " << gens.len << " generators:";
	if (f_tex) {
		ost << "\\\\" << endl;
		}
	else {
	...
	for (h = 0; h < nb_orbits; h++) {
		...
		if (f_tex) {
			...
			ost << " Orbit " << h << " / " << nb_orbits << " is " << i << " : " << orbit_len[i];
			ost << " : ";
			print_orbit(ost, i);
			ost << endl;
			}
...
void schreier::print_and_list_orbits_and_stabilizer_sorted_by_length(ostream &ost, INT f_tex, 
	action *default_action, longinteger_object &full_group_order)
{
	...
	for (h = 0; h < nb_orbits; h++) {
		i = Perm_inv[h];
		if (f_tex) {
			print_and_list_orbit_and_stabilizer_tex(i, default_action, full_group_order, ost);
			}
		else {
			ost << " Orbit " << h << " / " << nb_orbits << " is " << i << " : " << orbit_len[i];
			ost << " : ";
	...
	action *default_action, strong_generators *gens_full_group)
{
	...
	for (h = 0; h < nb_orbits; h++) {
		...
		if (f_tex) {
			//print_and_list_orbit_and_stabilizer_tex(i, default_action, full_group_order, ost);
			print_and_list_orbit_and_stabilizer_with_list_of_elements_tex(i, default_action, 
				gens_full_group, ost);
			}
		else {
			ost << " Orbit " << h << " / " << nb_orbits << " is " << i << " : " << orbit_len[i];
...
void schreier::print_and_list_orbits_of_given_length(ostream &ost, INT len)
{
	INT i;
	...
	for (i = 0; i < nb_orbits; i++) {
		...
		ost << " Orbit " << i << " / " << nb_orbits << " : " << orbit_len[i];
		ost << " : ";
		print_orbit(ost, i);
		ost << endl;
		}
...
void schreier::print_tables_latex(ostream &ost, INT f_with_cosetrep)
{
    INT i;
    int w; //  j, k;
...
void schreier::print_orbit_using_callback(ostream &ost, INT orbit_no, 
	void (*print_point)(ostream &ost, INT pt, void *data), void *data)
{
	INT i, first, len;
...
void schreier::transporter_from_orbit_rep_to_point(INT pt, INT &orbit_idx, INT *Elt, INT verbose_level)
{
	INT f_v = (verbose_level >= 1);
	INT pos;
	...
	if (f_v) {
		cout << "schreier::transporter_from_orbit_rep_to_point done" << endl;
		}
}

void schreier::transporter_from_point_to_orbit_rep(INT pt, INT &orbit_idx, INT *Elt, INT verbose_level)
{
	INT f_v = (verbose_level >= 1);
	INT pos;
	...
	//A->element_move(cosetrep, Elt, 0);
	if (f_v) {
		cout << "schreier::transporter_from_point_to_orbit_rep done" << endl;
		}
}
...
// to the j-th point in the orbit.
{
	...
	if (f_v) {
		cout << "schreier::coset_rep_with_verbosity j=" << j << " orbit[j]=" << orbit[j] << endl;
		}
	if (prev[j] != -1) {
		if (f_v) {
			cout << "schreier::coset_rep_with_verbosity j=" << j << " label[j]=" << label[j] << " orbit_inv[prev[j]]=" << orbit_inv[prev[j]] << endl;
			}
	...
	if (f_v) {
		cout << "schreier::coset_rep_with_verbosity j=" << j << " done" << endl;
		}
}
...
void schreier::compute_all_point_orbits(INT verbose_level)
{
	...
	if (f_v) {
		cout << "schreier::compute_all_point_orbits verbose_level=" << verbose_level << endl;
		}
	...
	for (pt0 = 0, pt = 0; pt < A->degree; pt++) {
		...
		if (f_vv) {
			cout << "schreier::compute_all_point_orbits pt = " << pt << " / " << A->degree << " nb_orbits=" << nb_orbits << " cur=" << cur << ", computing orbit" << endl;
			}
...
void schreier::compute_point_orbit(INT pt, INT verbose_level)
{
	...
	while (cur < total) {
		...
		for (i = 0; i < gens.len; i++) {
			...
			next_pt = get_image(cur_pt, i, 0 /*verbose_level - 5*/); // !!
				// A->element_image_of(cur_pt, gens.ith(i), FALSE);
			next_pt_loc = orbit_inv[next_pt];
			if (f_vv) {
...
void schreier::draw_tree(char *label, INT orbit_no, INT xmax, INT ymax, INT f_circletext, INT rad, 
	...
	INT verbose_level)
{
	...
	draw_tree2(label, xmax, ymax, f_circletext, weight, placement_x, max_depth, i, last, rad, 
	...
	INT verbose_level)
{
	...
	subtree_draw_vertices(G, f_circletext, x, y, weight, placement_x, max_depth, i, last, rad, 
		f_has_point_labels, point_labels, 
		verbose_level);
	...
	INT *placement_x, INT max_depth, INT i, INT last, INT verbose_level)
{
	...
	Px[1] = x;
	Py[1] = y;
	//cout << "schreier::subtree_draw_lines " << parent_x << "," << parent_y << " - " << x << "," << y << endl;
...
void schreier::get_orbit_partition_of_points_and_lines(partitionstack &S, INT verbose_level)
{
	...
	first_column_orbit = orbit_number(first_column_element); // orbit_no[pos];
...
strong_generators *schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal(action *default_action, 
	longinteger_object &full_group_order, INT pt, vector_ge *&cosets, INT verbose_level)
{
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal" << endl;
		}
	...
	transporter_from_point_to_orbit_rep(pt, orbit_index1, transporter, 0 /* verbose_level */);

	if (orbit_index1 != orbit_index) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal orbit_index1 != orbit_index" << endl;
		exit(1);
		}
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal before gens->init_generators_for_the_conjugate_group_aGav" << endl;
		}
	gens->init_generators_for_the_conjugate_group_aGav(gens0, 
		transporter, verbose_level);
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal after gens->init_generators_for_the_conjugate_group_aGav" << endl;
		}

	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal computing coset representatives" << endl;
		}
	for (i = 0; i < len; i++) {
		transporter_from_orbit_rep_to_point(orbit[fst + i], orbit_index1, transporter1, 0 /* verbose_level */);
		A->element_mult(transporter, transporter1, cosets->ith(i), 0);
		}
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal computing coset representatives done" << endl;
		}
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point_and_transversal done" << endl;
		}
	delete gens0;
	return gens;
}

strong_generators *schreier::generators_for_stabilizer_of_arbitrary_point(action *default_action, 
	longinteger_object &full_group_order, INT pt, INT verbose_level)
{
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point" << endl;
		}
	...
	if (orbit_index1 != orbit_index) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point orbit_index1 != orbit_index" << endl;
		exit(1);
		}
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point before gens->init_generators_for_the_conjugate_group_aGav" << endl;
		}
	gens->init_generators_for_the_conjugate_group_aGav(gens0, 
		transporter, verbose_level);
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point after gens->init_generators_for_the_conjugate_group_aGav" << endl;
		}

	FREE_INT(transporter);
	
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_arbitrary_point done" << endl;
		}
	delete gens0;
	return gens;
}
...
strong_generators *schreier::generators_for_stabilizer_of_orbit_rep(action *default_action, 
	longinteger_object &full_group_order, INT orbit_idx, INT verbose_level)
{
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_orbit_rep" << endl;
		}
	...
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_orbit_rep found a stabilizer group of order " << stab_order << endl;
		}
	...
	delete Stab;
	if (f_v) {
		cout << "schreier::generators_for_stabilizer_of_orbit_rep done" << endl;
		}
	return gens;
}
...
void schreier::get_orbit_reps(INT *&Reps, INT &nb_reps, INT verbose_level)
{
	INT f_v = (verbose_level >= 1);
	INT i, a, f;
...
void schreier::get_orbit_lengths_once_each(INT *&orbit_lengths, INT &nb_orbit_lengths)
{
	INT *val, *mult, len;	
...
INT schreier::orbit_number(INT pt)
{
	...
	if (orbit_first[idx] <= pos && pos < orbit_first[idx] + orbit_len[idx]) {
		return idx;
		}
	else {
		cout << "schreier::orbit_number something is wrong" << endl;
		exit(1);
		}
}
...
void schreier::get_orbit_decomposition_scheme_of_graph(INT *Adj, INT n, INT *&Decomp_scheme, INT verbose_level)
{
	INT f_v = (verbose_level >= 1);
	INT I, J;
	INT f1, l1;
		...
		for (J = 0; J < nb_orbits; J++) {
			...
			for (i = 0; i < l1; i++) {
				...
				if (i == 0) {
					...
					if (r0 != r) {
						cout << "schreier::get_orbit_decomposition_scheme_of_graph not tactical" << endl;
						cout << "I=" << I << endl;
						cout << "J=" << J << endl;
						cout << "r0=" << r0 << endl;
						cout << "r=" << r << endl;
	...
	if (f_v) {
		cout << "schreier::get_orbit_decomposition_scheme_of_graph done" << endl;
		}
}
