...
void analyze(INT n, INT k, INT t, INT q, const BYTE *fname, INT verbose_level)
{
	...
	for (i = 0; i < nb_designs; i++) {
		...
		analyze_design(n, k, t, q, F, Gr_t, Gr_k, Gr_tk, Designs + i * len, len, verbose_level);
		}
...
#if 0
		cout << "subspace " << j << " / " << len << " is generated by:" << endl;
...
#if 0
		cout << "looping over the " << kCt << " subspaces of dimension " << t << " of it:" << endl;
#endif

		for (h = 0; h < kCt; h++) {
			GE->unrank_embedded_INT(subspace_basis_with_embedding, h, 0 /* verbose_level */);
				// subspace_basis_with_embedding is n x big_n (i.e., k x n here)
			rk = Gr_t->rank_INT_here(subspace_basis_with_embedding, 0 /* verbose_level */);
#if 0
...
void test_group(INT n, INT k, INT t, INT q, const BYTE *fname, linear_group_description *Descr, INT verbose_level)
{
	...
	cout << "test_group after LG->init, strong generators for the group have been created" << endl;
	...
	for (i = 0; i < nb_designs; i++) {
		cout << "analyzing design " << i << " / " << nb_designs << ":" << endl;
		group_of_design(n, k, t, q, F, Gr_t, Gr_k, Gr_tk, A, A2, Designs + i * len, len, verbose_level);
		}
	...
	INT *design, INT len, INT verbose_level)
{
	...
	if (f_v) {
		cout << "group_of_design initializing set_stabilizer_compute:" << endl;
		cout << "A=";
		A->print_info();
		cout << "A2=";
		A2->print_info();
	...
	S->init_with_strong_generators(A2, A, 
		A->Strong_gens, 
		design /* the_set */, len /*set_size*/, verbose_level);
