...
{
	ex ed = e.diff(x, nth);
	cout << "derivative of " << e << " is " << ed << endl;
	if (!(ed - d).is_zero()) {
		switch (nth) {
		case 0:
			clog << "zeroth ";
...
static unsigned exam_differentiation6()
{
	...
	cout << "Taylor series of sin(x) at 0, degree 8: " << e << endl;
	d = cos(x).series(x==0, 7);
	cout << "Taylor series of cos(x) at 0, degree 7: " << e << endl;
	ed = e.diff(x);
	ed = series_to_poly(ed);
	cout << "derivative of Taylor series of sin(x) at 0, degree 8: " << ed << endl;
	d = series_to_poly(d);
	cout << "Taylor series of cos(x) at 0, degree 7, as polynomials: " << d << endl;
