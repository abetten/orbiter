...
void wreath_product_orbits_CUDA(wreath_product* W,
	...
	linalg::Matrix<int> M (W->degree_of_tensor_action, mtx_n);
	...
	linalg::Matrix<int> N (SG->gens->len * mtx_n, mtx_n);
...
//	linalg::Matrix<int> N ;
//	N.set_data_reference (generator_stack, SG->gens->len * mtx_n, mtx_n);
//	N.dealloc_data_on_destruct = false;

	// MN = M * N:
	linalg::Matrix<int> MN (M.nrows, N.ncols);
