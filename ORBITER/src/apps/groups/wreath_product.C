...
void do_it(int argc, const char **argv, INT nb_factors, INT n, INT q, INT verbose_level);
		...
		INT verbose_level)
{
	...
	A->init_wreath_product_group_and_restrict(nb_factors, n,
			F,
			verbose_level);
	cout << "tensor_product::init after A->init_wreath_product_group_and_restrict" << endl;

	if (!A->f_has_subaction) {
		cout << "tensor_product::init action A does not have a subaction" << endl;
		exit(1);
	}
	A0 = A->subaction;
	...
	if (!A0->f_has_strong_generators) {
		cout << "tensor_product::init action A0 does not have strong generators" << endl;
		exit(1);
		}
	...
	for (i = 0; i < SG->gens->len; i++) {
		...
		A->element_print_as_permutation_with_offset(SG->gens->ith(i), cout,
				0 /* offset*/, TRUE /* f_do_it_anyway_even_for_big_degree*/,
				TRUE /* f_print_cycles_of_length_one*/, 0 /* verbose_level*/);
...
void do_it(int argc, const char **argv,
	INT nb_factors, INT n, INT q, INT verbose_level)
{
	//INT f_v = (verbose_level >= 1);
	finite_field *F;
	action *A;
	...
	A->init_wreath_product_group_and_restrict(nb_factors, n,
			F,
			verbose_level);
	cout << "after A->init_wreath_product_group_and_restrict" << endl;

	if (!A->f_has_subaction) {
		cout << "action A does not have a subaction" << endl;
		exit(1);
	}
	A0 = A->subaction;
	...
	if (!A0->f_has_strong_generators) {
		cout << "action A0 does not have strong generators" << endl;
		exit(1);
		}
	strong_generators *SG;
	longinteger_object go;
	//wreath_product *W = A0->G.wreath_product_group;
