...
// called from callback_print_isomorphism_type()
{
	...
	if (k == 2) {
		klein(file, /*file_klein,*/ Iso, iso_cnt,
				Stab, Orb, data, Iso->size,  verbose_level);
		}

	file << "The set of " << k - 1 << "-Subspaces in "
			"PG($" << n - 1 << ", " << q << "$) is:\\\\" << endl;
	
	for (i = 0; i < Iso->size; i++) {
		file << "$S_{" << i + 1 << "}$ has rank " << data[i]
	...
	file << "The stabilizer of order " << so
...
void spread_classify::report2(isomorph &Iso, int verbose_level)
{
	...
	f << "\\hline" << endl;
	f << "ID & Isom. Type & $|\\mbox{Aut}|$ & $|\\mbox{Aut}|$ "
	...
	for (h = 0; h < Iso.Reps->count; h++) {
		...
		for (i = 0; i < Orb.nb_orbits; i++) {
			...
			cout << "inducing action on orbit " << i
					<< " of length " << len << endl;
			f << "Generators for the kernel in the action "
					"on orbit " << i << " are:";
			long int *set;
			action *A1;
			vector_ge *gens;
		...
		ofstream &f, int h, int &cnt, int verbose_level)
{
	...
	for (i = 0; i < order + 1; i++) {
		Grass->unrank_lint_here(Mtx, data[i], 0/*verbose_level - 4*/);
	...
	for (u = 0; u < Orb.nb_orbits; u++) {
		...
		for (i = 0; i < order; i++) {
			a = data[List[i]];
			Grass->unrank_lint_here(Mtx, a, 0/*verbose_level - 4*/);
			...
			b = Gr->rank_lint_here(Mtx, 0/*verbose_level - 4*/);
			data2[i] = b;
			}
		...
		int *S, void *data, int verbose_level)
{
	...
	return Spread->check_function(len, S, verbose_level);
}
#endif
