...
// Called from print_isomorphism_type if k == 2
{
	...
	// compute TDO:
	{
		...
		sprintf(fname_row_scheme, "%s_%d_planes_row_scheme.tex", Iso->prefix, iso_cnt);
		sprintf(fname_col_scheme, "%s_%d_planes_col_scheme.tex", Iso->prefix, iso_cnt);
		{
		...
		I->get_and_print_row_tactical_decomposition_scheme_tex(
			fp_row_scheme, FALSE /* f_enter_math */, TRUE /* f_print_subscripts */, *Stack);
		I->get_and_print_column_tactical_decomposition_scheme_tex(
			fp_col_scheme, FALSE /* f_enter_math */, TRUE /* f_print_subscripts */, *Stack);
		}
...
void spread::make_spread_from_q_clan(int *data, int type_of_spread, int verbose_level)
{
	...
	int sz = order + 1;
	int M[8];
	int h, h1, i, s, t, t2, t3, t4, t5, t7, t9, a_t = 0, b_t = 0, c_t = 0, x, y, w, z, r;
