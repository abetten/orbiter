...
coltitle=black,title=Creating Diophantine Systems]
		...
		\hline
		\verb'-RHS' & options  & Set RHS info for a set of $m$ rows. Possible types of conditions are EQ, LE, INT, ZOR. This command can be repeated. 
		Example: mult=2,EQ=1 is condition for 2 rows where the RHS is equal to zero. 
		Example: mult=1,LE=3 is one condition where the RHS is at most three. 
		Example: mult=1,INT=1to2 is one condition where the RHS is between 1 and 2. 
		Example: mult=1,ZOR=2 is one condition where the RHS is either 0 or 2. 
		\\
		\hline
		...
		\hline
		\end{tcolorbox}
	\caption{\label{tab:diophant}Creating Diophantine Systems}
	\end{table}
