	...
	int f_print_isot_small, int f_print_isot, int verbose_level)
{
	...
	if (f_print_isot) {
		printf("Aut: "); fflush(stdout);
	}
	else if (f_print_isot_small) {
		printf("aut = "); fflush(stdout);
	}
#if 0
	if (fp) {
	...
	info.f_use_ddb = FALSE;
#if 0
	iso_info_init_ddp(&info,
		isot->f_ddp, get_ddp(pc),
		get_ddp(pc));
	iso_info_init_ddb(&info,
		isot->f_ddb, get_ddb(pc),
		get_ddb(pc));
#endif
	...
	if (f_print_isot) {
		print_aut_group_order(
			info.nb_isomorphisms);
		}
	else if (f_print_isot_small) {
		printf("%d ", info.nb_isomorphisms);
		}
	fflush(stdout);
#if 0
	if (fp) {
		if (f_print_isot) {
			fprint_aut_group_order(
			fp, info.nb_isomorphisms);
		}
		else if (f_print_isot_small) {
			fprintf(fp, "%d ",
			info.nb_isomorphisms);
