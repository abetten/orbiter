	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		if (strcmp(argv[i], "-q") == 0) {
			...
			make_table_of_irreducible_polynomials_degree = atoi(argv[++i]);
		...
		else if (strcmp(argv[i], "-cheat_sheet_PG") == 0) {
			f_cheat_sheet_PG = TRUE;
			cheat_sheet_PG_n = atoi(argv[++i]);
			...
			canonical_form_PG_n = atoi(argv[++i]);
