...
void semifield_level_two::downstep(int verbose_level)
{
	...
	for (i = 0; i < nb_classes; i++) {
		...
		if (f_vv) {
			...
			Orbiter->Int_vec->matrix_print(class_rep_plus_I_Basis[i], k, k);
		}
		...
		if (f_vv) {
			...
			Orbiter->Int_vec->matrix_print(class_rep_plus_I_Basis_inv[i], k, k);
		}
