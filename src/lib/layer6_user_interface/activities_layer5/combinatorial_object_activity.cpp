		...
		int verbose_level)
{
	...
	if (Descr->f_canonical_form_PG) {
		...
		if (f_v) {
			cout << "combinatorial_object_activity::perform_activity_combo "
					"after Combo->do_canonical_form_and_write_files" << endl;
		}
		...
		layer5_applications::canonical_form::combinatorial_object_with_properties *OwP;
	...
	if (Descr->f_report) {
		...
		layer5_applications::orbits::orbits_create *Orbits;
		...
		cout << "combinatorial_object_activity::perform_activity_combo "
				"Combo->do_activity is not available"
				<< endl;
		exit(1);
