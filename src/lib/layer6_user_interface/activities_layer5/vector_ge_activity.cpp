		...
		int verbose_level)
{
	...
	if (Descr->f_report) {
		...
		conjugate_data = NEW_int(vec[0]->A->elt_size_in_int);

		vec[0]->A->Group_element->code_for_make_element(
				conjugate_data, vec[1]->ith(0));


		Action_global.conjugate_and_write_to_file(
				vec[0]->A,
				label_in,
				conjugate_data,
				vec[0],
	...
	else if (Descr->f_conjugate_by) {

		if (f_v) {
			cout << "vector_ge_activity::perform_activity "
					"f_conjugate_by " << Descr->conjugate_by_data << endl;
		}
		...
		Elt = NEW_int(vec[0]->A->elt_size_in_int);

		conjugate_data = NEW_int(vec[0]->A->make_element_size);


		vec[0]->A->Group_element->make_element_from_string(
				Elt, Descr->conjugate_by_data, verbose_level);
		...
		if (f_v) {
			cout << "vector_ge_activity::perform_activity "
					"before Action_global.conjugate_and_write_to_file" << endl;
		}
		Action_global.conjugate_and_write_to_file(
				vec[0]->A,
				label_in,
		...
		if (f_v) {
			cout << "vector_ge_activity::perform_activity "
					"after Action_global.conjugate_and_write_to_file" << endl;
		}
		...
		layer5_applications::apps_algebra::vector_ge_builder *VB0;
		layer5_applications::apps_algebra::vector_ge_builder *VB1;

		VB0 = NEW_OBJECT(layer5_applications::apps_algebra::vector_ge_builder);
		VB1 = NEW_OBJECT(layer5_applications::apps_algebra::vector_ge_builder);
		...
		if (f_v) {
			cout << "vector_ge_activity::perform_activity "
					"before Action_global.apply_isomorphism_wedge_product_4to6_and_write_file" << endl;
		}
		...
		if (f_v) {
			cout << "vector_ge_activity::perform_activity "
					"after Action_global.apply_isomorphism_wedge_product_4to6_and_write_file" << endl;
		}
