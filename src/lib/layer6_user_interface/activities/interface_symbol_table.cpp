		...
		int argc, std::string *argv, int &i, int verbose_level)
{
	...
	if (ST.stringcmp(argv[i], "-define") == 0) {
		...
		f_define = true;
		Symbol_definition = NEW_OBJECT(layer5_applications::user_interface::core_system::symbol_definition);
		...
		Symbol_definition->read_definition(layer5_applications::user_interface::core_system::The_Orbiter_top_level_session, argc, argv, i, verbose_level);
		if (f_v) {
			cout << "interface_symbol_table::read_arguments "
		...
		int argc, std::string *argv, int &i, int verbose_level)
{
	...
	while (true) {
		...
		if (ST.stringcmp(argv[i], "-and") == 0) {
			...
			f_activity = true;
			Activity_description = NEW_OBJECT(layer6_user_interface::control_everything::activity_description);
