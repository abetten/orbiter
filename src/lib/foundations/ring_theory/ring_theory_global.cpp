...
void ring_theory_global::create_ideal(
		field_theory::finite_field *F,
...
// creates homogeneous_polynomial_domain
{
	...
	if (f_v) {
		cout << "ring_theory_global::create_ideal" << endl;
	}
	...
	HPD->init(F, ideal_nb_vars, ideal_degree,
			FALSE /* f_init_incidence_structure */,
			Monomial_ordering_type,
			verbose_level);


	nb_monomials = HPD->get_nb_monomials();

	HPD->print_monomial_ordering(cout);

	label_txt.assign(ideal_label);
	label_tex.append(ideal_label_tex);

	long int *Pts;
	int nb_pts;

	if (f_v) {
		cout << "ring_theory_global::create_ideal "
	...
	//coeffs = NEW_lint(HPD->get_P()->N_points);
	...
	if (f_v) {
		cout << "ring_theory_global::create_ideal done" << endl;
	}
}
