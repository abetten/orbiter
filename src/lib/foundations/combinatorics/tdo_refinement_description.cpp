...
int tdo_refinement_description::read_arguments(int argc, std::string *argv, int verbose_level)
{
	...
	for (i = 0; i < argc - 1; i++) {
		if (stringcmp(argv[i], "-lambda3") == 0) {
			f_lambda3 = TRUE;
			lambda3 = strtoi(argv[++i]);
			block_size = strtoi(argv[++i]);
			cout << "-lambda3 " << lambda3 << " " << block_size << endl;
		}
		if (stringcmp(argv[i], "-scale") == 0) {
			f_scale = TRUE;
			scaling = strtoi(argv[++i]);
		...
		else if (stringcmp(argv[i], "-range") == 0) {
			f_range = TRUE;
			range_first = strtoi(argv[++i]);
			range_len = strtoi(argv[++i]);
			cout << "-range " << range_first << " " << range_len << endl;
		}
		else if (stringcmp(argv[i], "-select") == 0) {
			f_select = TRUE;
			select_label.assign(argv[++i]);
			cout << "-select " << select_label << endl;
		}
		else if (stringcmp(argv[i], "-o1") == 0) {
			f_omit1 = TRUE;
			omit1 = strtoi(argv[++i]);
			cout << "-o1 " << omit1 << endl;
		}
		else if (stringcmp(argv[i], "-o2") == 0) {
			f_omit2 = TRUE;
			omit2 = strtoi(argv[++i]);
			cout << "-o2 " << omit2 << endl;
		}
		if (stringcmp(argv[i], "-D1_upper_bound_x0") == 0) {
			f_D1_upper_bound_x0 = TRUE;
			D1_upper_bound_x0 = strtoi(argv[++i]);
			cout << "-D1_upper_bound_x0 " << D1_upper_bound_x0 << endl;
		}
		else if (stringcmp(argv[i], "-reverse") == 0) {
			f_reverse = TRUE;
			cout << "-reverse" << endl;
		}
		else if (stringcmp(argv[i], "-reverse_inverse") == 0) {
			f_reverse_inverse = TRUE;
			cout << "-reverse_inverse" << endl;
		}
		else if (stringcmp(argv[i], "-nopacking") == 0) {
			f_use_packing_numbers = FALSE;
			cout << "-nopacking" << endl;
		}
		else if (stringcmp(argv[i], "-dual_is_linear_space") == 0) {
			f_dual_is_linear_space = TRUE;
			cout << "-dual_is_linear_space" << endl;
		}
		else if (stringcmp(argv[i], "-geometric_test") == 0) {
			f_do_the_geometric_test = TRUE;
			cout << "-geometric_test" << endl;
		}
		else if (stringcmp(argv[i], "-once") == 0) {
			f_once = TRUE;
			cout << "-once" << endl;
		}
		else if (stringcmp(argv[i], "-mckay") == 0) {
			f_use_mckay_solver = TRUE;
			cout << "-mckay" << endl;
		}
		else if (stringcmp(argv[i], "-input_file") == 0) {
			f_input_file = TRUE;
