	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		...
		else if (strcmp(argv[i], "-test_single_equation") == 0) {
			f_test_single_equation = TRUE;
			max_number_of_coefficients = atoi(argv[++i]);
			cout << "-test_single_equation " << max_number_of_coefficients << endl;
		}
		else if (strcmp(argv[i], "-project_to_single_equation_and_solve") == 0) {
			f_project_to_single_equation_and_solve = TRUE;
			eqn_idx = atoi(argv[++i]);
			solve_case_idx = atoi(argv[++i]);
			cout << "-project_to_single_equation_and_solve " << eqn_idx << " " << solve_case_idx << endl;
		}
		else if (strcmp(argv[i], "-project_to_two_equations_and_solve") == 0) {
			f_project_to_two_equations_and_solve = TRUE;
			eqn1_idx = atoi(argv[++i]);
			eqn2_idx = atoi(argv[++i]);
			solve_case_idx_r = atoi(argv[++i]);
			solve_case_idx_m = atoi(argv[++i]);
