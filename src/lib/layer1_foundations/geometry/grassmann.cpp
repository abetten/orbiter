		...
		int verbose_level)
{
	...
	if (f_v) {
		cout << "grassmann::unrank_longinteger "
				"coset " << b << " = ";
		Int_vec_print(cout, coset, nb_free_cols);
		cout << endl;
	}
		...
		int verbose_level)
{
	...
	if (k1 != k) {
		cout << "grassmann::rank_longinteger "
				"error, does not have full rank" << endl;
		exit(1);
	}
	if (f_v) {
	...
	if (f_v) {
		cout << "grassmann::rank_longinteger "
				"calling int_vec_complement for ";
		Int_vec_print(cout, base_cols, k);
		cout << endl;
	}
	orbiter_kernel_system::Orbiter->Int_vec->complement(base_cols, n, k);
	...
	// rank the coset:
	if (nb_free_cols) {
		Gg.AG_element_rank_longinteger(
				q, coset, 1, nb_free_cols, b);
		if (f_v) {
	...
	// and the rank c of the subspace:
	if (f_v) {
		cout << "grassmann::rank_longinteger computing "
				"r:=" << r << " + " << b
				<< " * " << a << " + " << c << endl;
	}
	D.mult(b, a, tmp1);
		...
		int verbose_level)
{
	...
	for (a = 0; a < nb_lines; a++) {
		...
		F->Linear_algebra->perp_standard(
				n, k, Basis, 0 /*verbose_level*/);
		if (f_vv) {
			cout << "after perp:" << endl;
			Int_matrix_print(Basis, n, n);
		}
		b = rank_lint_here(
				Basis + k * n, 0/*verbose_level - 4*/);
		if (f_vv) {
		...
		int verbose_level)
{
	...
	if (f_v) {
		cout << "grassmann::compute_dual_spread "
				"The dual spread is : ";
		...
		int verbose_level)
{
	...
	a = rank_lint(0/*verbose_level - 4*/);
	if (f_v3) {
		cout << "grassmann::make_special_element_infinity "
				"a=" << a << endl;
	}
	return a;
}
		...
		int verbose_level)
{
	...
	if (f_v) {
		cout << "grassmann::make_spread_from_spread_set, "
				"sz = " << sz << endl;
	}
