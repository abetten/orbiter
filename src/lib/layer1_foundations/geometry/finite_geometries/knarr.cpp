...
// creates a W3q and a projective space P5 and a grassmann G63
{
	...
	W = NEW_OBJECT(W3q);
	P5 = NEW_OBJECT(projective_geometry::projective_space);
	...
	G63 = NEW_OBJECT(projective_geometry::grassmann);
		...
		int verbose_level)
{
	...
	for (h = 0; h < q + 1; h++) {
		...
		projective_geometry::grassmann *G32;
		projective_geometry::grassmann_embedded *Gre;

		
		G32 = NEW_OBJECT(projective_geometry::grassmann);
		Gre = NEW_OBJECT(projective_geometry::grassmann_embedded);
