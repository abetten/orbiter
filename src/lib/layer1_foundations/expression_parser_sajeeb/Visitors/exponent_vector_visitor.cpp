...
//    if (op_node->coefficient_vector.size() == 0)
//        op_node->coefficient_vector.resize(symbol_table->size(), 0);
//
//    for (auto it=op_node->children.begin(); it!=op_node->children.end(); ++it)
//        (*it)->accept(this, op_node->coefficient_vector, it);
//    monomial_coefficient_table_.insert({op_node->coefficient_vector, op_node});
//
//    // remove the managed variables
//    for (auto it=op_node->children.begin(); it!=op_node->children.end(); ++it)
//        if ((*it)->type == irtree_node::node_type::VARIABLE_NODE)
...
void exponent_vector_visitor::visit(multiply_node* op_node,
    ...
    for (shared_ptr<irtree_node>& child : op_node->children)
        child->accept(this, op_node->coefficient_vector);
    monomial_coefficient_table_.insert({op_node->coefficient_vector, op_node});
...
void exponent_vector_visitor::visit(multiply_node* op_node, list<shared_ptr<irtree_node> >::iterator& link) {
    op_node->accept(this, op_node->coefficient_vector, link);
}
