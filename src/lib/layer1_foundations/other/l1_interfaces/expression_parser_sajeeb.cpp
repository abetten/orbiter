...
static void convert_to_orbiter_recursion(
		...
		algebra::expression_parser::syntax_tree_node *current_node_copy,
		int verbose_level);
		...
		int verbose_level)
{
	...
	if (f_v) {
		...
		cout << "expression_parser_sajeeb::convert_to_orbiter "
				"f_is_commutative = " << f_is_commutative << endl;
	}
	...
	if (root_node->type == irtree_node::node_type::SENTINEL_NODE) {
		...
		Tree = NEW_OBJECT(algebra::expression_parser::syntax_tree);

		if (f_v) {
			cout << "expression_parser_sajeeb::convert_to_orbiter "
					"before Tree->init" << endl;
		}
		Tree->init(F, f_is_commutative, f_has_managed_variables, managed_variables, verbose_level);
		if (f_v) {
			cout << "expression_parser_sajeeb::convert_to_orbiter "
					"after Tree->init" << endl;
		}

		Tree->Root = NEW_OBJECT(algebra::expression_parser::syntax_tree_node);
		...
		int verbose_level)
{
	...
	for (const shared_ptr<irtree_node>& child : current_node->children) {
		...
		if (child.get()->type == irtree_node::node_type::SENTINEL_NODE) {
			...
			algebra::expression_parser::syntax_tree_node *fresh_node;
...
// current_node_copy is addition node
{
	...
	for (const shared_ptr<irtree_node>& child : current_node->children) {
		...
		if (child.get()->type == irtree_node::node_type::SENTINEL_NODE) {
			...
			algebra::expression_parser::syntax_tree_node *artificial_mult_node;
