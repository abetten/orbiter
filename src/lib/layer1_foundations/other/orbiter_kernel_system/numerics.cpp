		...
		int verbose_level)
{
	...
	algebra::ring_theory::polynomial_double_domain Poly;
	algebra::ring_theory::polynomial_double *P;
	algebra::ring_theory::polynomial_double *det;

	Poly.init(n);
	P = NEW_OBJECTS(algebra::ring_theory::polynomial_double, n * n);
	for (i = 0; i < n; i++) {
		for (j = 0; j < n; j++) {
			P[i * n + j].init(n + 1);
			if (i == j) {
	...
	det = NEW_OBJECT(algebra::ring_theory::polynomial_double);
	det->init(n + 1);
