...
algebra::field_theory::finite_field *unipoly_domain::get_F()
{
	return F;
}
	...
	int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::create_object_by_rank "
				"rk=" << rk << endl;
		cout << "unipoly_domain::create_object_by_rank "
				"f_factorring=" << f_factorring << endl;
	}
...
void unipoly_domain::create_object_from_table_of_coefficients(
	...
	int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::create_object_from_table_of_coefficients" << endl;
	}
	if (f_factorring) {
		cout << "unipoly_domain::create_object_from_table_of_coefficients "
					"it should not be a factorring" << endl;
		exit(1);
	}


	int *rep = NEW_int(coeff_table_len + 1);
	rep[0] = coeff_table_len - 1;
	int *coeff = rep + 1;
	int i = 0;

	for (i = 0; i < coeff_table_len; i++) {
		coeff[i] = coeff_table[i];
	}
	p = (void *) rep;
}
...
int unipoly_domain::recalculate_degree(
		unipoly_object p)
{
	int *rep = (int *) p;
	int d = rep[0]; // degree
...
void unipoly_domain::scalar_multiply(
		unipoly_object a, int c)
{
	int *ra = (int *) a;
	if (ra == NULL) {
		cout << "unipoly_domain::scalar_multiply rep == NULL" << endl;
		exit(1);
	}
	int m = ra[0];
	int *A = ra + 1;
...
// the j-th row of Frob is x^{j*q} mod m
{
	...
	d = recalculate_degree(factor_polynomial);
...

{
	...
	factor_polynomial_degree = recalculate_degree(factor_polynomial);
	if (f_v) {
		cout << "unipoly_domain::Frobenius_matrix "
	...
	create_object_by_rank(a, F->q, 0 /*verbose_level*/); // the polynomial X
	create_object_by_rank(b, 1, 0 /*verbose_level*/); // the polynomial 1
	create_object_by_rank(c, 0, 0 /*verbose_level*/);
	create_object_by_rank(m_mod, 0, 0 /*verbose_level*/);
	create_object_by_rank(Q, 0, 0 /*verbose_level*/);
	create_object_by_rank(R, 0, 0 /*verbose_level*/);
	...
	int verbose_level)
{
...

	{
		...
		for (i = da, j = dq; i >= db; i--, j--) {
			...
			if (R[i] != 0) {
				cout << "unipoly_domain::division_with_remainder: R[i] != 0" << endl;
				exit(1);
			}
			//cout << "i=" << i << endl;
...
{
	int dm = recalculate_degree(m);
	int dn = recalculate_degree(n);
	...
	unipoly_object &g, int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::greatest_common_divisor m=";
		print_object(m, cout);
		cout << " n=";
		print_object(n, cout);
		cout << endl;
	...
	if (f_v) {
		cout << "unipoly_domain::greatest_common_divisor "
	...
	if (c == 0 && is_zero(n)) {
		...
		if (f_v) {
			cout << "unipoly_domain::greatest_common_divisor of m=";
			print_object(m, cout);
			cout << " n=";
			print_object(n, cout);
			cout << " is ";
	...
	create_object_by_rank(M, 0, 0 /*verbose_level*/);
	create_object_by_rank(N, 0, 0 /*verbose_level*/);
	create_object_by_rank(Q, 0, 0 /*verbose_level*/);
	create_object_by_rank(R, 0, 0 /*verbose_level*/);
	...
	while (true) {
		...
		if (f_vv) {
			cout << "unipoly_domain::greatest_common_divisor Q=";
			print_object(Q, cout);
			cout << " R=";
			print_object(R, cout);
			cout << endl;
	...
	if (f_v) {
		cout << "unipoly_domain::greatest_common_divisor g=";
		print_object(g, cout);
		cout << endl;
	}
	...
	if (f_v) {
		cout << "unipoly_domain::greatest_common_divisor done" << endl;
	}
	...
	unipoly_object &g, int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::extended_gcd" << endl;
	}
	if (f_vv) {
		cout << "m=";
		print_object(m, cout);
	...
	if (c < 0) {
		if (f_v) {
			cout << "unipoly_domain::extended_gcd before extended_gcd" << endl;
		}
		extended_gcd(n, m, v, u, g, verbose_level);
		if (f_v) {
			cout << "unipoly_domain::extended_gcd after extended_gcd" << endl;
		}
		if (f_v) {
			cout << "unipoly_domain::extended_gcd done" << endl;
		}
		return;
	}
	if (f_v) {
		cout << "unipoly_domain::extended_gcd before assign m,u" << endl;
	}
	assign(m, u, 0 /*verbose_level*/);
	if (f_v) {
		cout << "unipoly_domain::extended_gcd before assign n,v" << endl;
	}
	...
	if (c == 0 || is_zero(n)) {
		...
		zero(v);
		if (f_v) {
			cout << "unipoly_domain::extended_gcd before assign m,g" << endl;
		}
		assign(m, g, 0 /*verbose_level*/);
		if (f_v) {
			cout << "unipoly_domain::extended_gcd done" << endl;
	...
	create_object_by_rank(Q, 0, 0 /*verbose_level*/);
	create_object_by_rank(R, 0, 0 /*verbose_level*/);
	create_object_by_rank(u1, 1, 0 /*verbose_level*/);
	create_object_by_rank(u2, 0, 0 /*verbose_level*/);
	create_object_by_rank(u3, 0, 0 /*verbose_level*/);
	create_object_by_rank(v1, 0, 0 /*verbose_level*/);
	create_object_by_rank(v2, 1, 0 /*verbose_level*/);
	create_object_by_rank(v3, 0, 0 /*verbose_level*/);
	create_object_by_rank(tmp, 0, 0 /*verbose_level*/);
	
	if (f_v) {
		cout << "unipoly_domain::extended_gcd before assign m,M" << endl;
	}
	assign(m, M, 0 /*verbose_level*/);
	if (f_v) {
		cout << "unipoly_domain::extended_gcd before assign n,N" << endl;
	}
	assign(n, N, 0 /*verbose_level*/);
	...
	while (true) {
		if (f_v) {
			cout << "unipoly_domain::extended_gcd iteration " << cnt << endl;
		}
		if (f_v) {
			cout << "unipoly_domain::extended_gcd M=";
			print_object(M, cout);
			cout << endl;
			cout << "unipoly_domain::extended_gcd N=";
			print_object(N, cout);
			cout << endl;
		}
		if (f_v) {
			cout << "unipoly_domain::extended_gcd before division_with_remainder" << endl;
		}
		division_with_remainder(M, N, Q, R, 0 /*verbose_level - 2*/);
		if (f_v) {
			cout << "unipoly_domain::extended_gcd after division_with_remainder" << endl;
		}
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd Q=";
			print_object(Q, cout);
			cout << endl;
			cout << "unipoly_domain::extended_gcd R=";
			print_object(R, cout);
			cout << endl;
		}
		if (is_zero(R)) {
		...
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before negate(Q)" << endl;
		}
		negate(Q);

		// u3 := u1 - Q * u2
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before mult(Q, u2, tmp)" << endl;
		}
		mult(Q, u2, tmp, 0 /*verbose_level - 1*/);
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before add(u1, tmp, u3)" << endl;
		}
		add(u1, tmp, u3);
		
		// v3 := v1 - Q * v2
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before mult(Q, v2, tmp)" << endl;
		}
		mult(Q, v2, tmp, 0 /*verbose_level - 1*/);
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before add(v1, tmp, v3)" << endl;
		}
		add(v1, tmp, v3);
		
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before assign(N, M)" << endl;
		}
		assign(N, M, 0 /*verbose_level*/);
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before assign(R, N)" << endl;
		}
		assign(R, N, 0 /*verbose_level*/);
		if (f_vv) {
			cout << "unipoly_domain::extended_gcd before assign(u2, u1)" << endl;
		}
	...
	assign(N, g, 0 /*verbose_level*/);
	if (f_v) {
		cout << "unipoly_domain::extended_gcd g=";
		print_object(g, cout);
		cout << "unipoly_domain::extended_gcd u=";
		print_object(u, cout);
		cout << "unipoly_domain::extended_gcd v=";
		print_object(v, cout);
		cout << endl;
	}
	if (f_v) {
		cout << "unipoly_domain::extended_gcd number of iterations = " << cnt << endl;
	}
		...
		unipoly_object p, int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::is_squarefree" << endl;
	}
	create_object_by_rank(a, 0, 0 /*verbose_level*/);
	create_object_by_rank(b, 0, 0 /*verbose_level*/);
	create_object_by_rank(u, 0, 0 /*verbose_level*/);
	create_object_by_rank(v, 0, 0 /*verbose_level*/);
	create_object_by_rank(g, 0, 0 /*verbose_level*/);
	
	assign(p, a, 0 /*verbose_level*/);
	if (f_v) {
		cout << "unipoly_domain::is_squarefree "
	...
	if (f_v) {
		cout << "unipoly_domain::is_squarefree done" << endl;
	}
	if (d >= 1) {
		return false;
	}
	...
	int verbose_level)
{
	...
	create_object_by_rank(mue, 0, 0 /*verbose_level*/);
	create_object_by_rank(lambda, 0, 0 /*verbose_level*/);
	create_object_by_rank(GCD, 0, 0 /*verbose_level*/);
	create_object_by_rank(Q, 0, 0 /*verbose_level*/);
	create_object_by_rank(R, 0, 0 /*verbose_level*/);
	create_object_by_rank(R1, 0, 0 /*verbose_level*/);
	create_object_by_rank(R2, 0, 0 /*verbose_level*/);
	...
	while (recalculate_degree(mue) < deg) {
		i++;
		if (f_v) {
			cout << "unipoly_domain::compute_normal_basis "
					"i = " << i << " / " << deg << endl;
		...
		if (recalculate_degree(GCD) < recalculate_degree(lambda)) {
		...
		int verbose_level)
{
	...
	factor_polynomial_degree = recalculate_degree(a);
	
	if (f_v) {
		cout << "unipoly_domain::is_irreducible "
				"before is_squarefree" << endl;
	}
	...
	if (f_v) {
		cout << "unipoly_domain::is_irreducible "
				"before Berlekamp_matrix" << endl;
	}
	Berlekamp_matrix(B, a, verbose_level - 2);
	if (f_v) {
		cout << "unipoly_domain::is_irreducible "
				"after Berlekamp_matrix" << endl;
	}
	if (f_vv) {
		cout << "unipoly_domain::is_irreducible "
				"Berlekamp_matrix=" << endl;
	...
	if (f_v) {
		cout << "unipoly_domain::is_irreducible "
				"Berlekamp_matrix has rank " << r << endl;
	}
	...
	int f, int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::get_a_primitive_polynomial" << endl;
	...
	if (f_vv) {
		cout << "unipoly_domain::get_a_primitive_polynomial "
				"after factor_into_longintegers "
	...
	int f, int verbose_level)
{
	...
	if (f_v) {
		cout << "unipoly_domain::get_an_irreducible_polynomial" << endl;
	...
	while (true) {
		...
		if (f_vv) {
			cout << "unipoly_domain::get_an_irreducible_polynomial "
				"candidate " << current << " : ";
			print_object(x, cout);
			cout << endl;
		}
...
// does not mod out by factor polynomial
{
	...
	//cout << " n=" << n << endl;
	create_object_by_rank(b, 0, 0 /*verbose_level*/);
	create_object_by_rank(c, 1, 0 /*verbose_level*/); // c = 1
	create_object_by_rank(d, 0, 0 /*verbose_level*/);
		...
		if (n % 2) {
			if (f_vv) {
				cout << "unipoly_domain::power_int "
						"n is odd" << endl;
				cout << "unipoly_domain::power_int "
						"before mult(b,c,d)" << endl;
			}
			mult(b, c, d, verbose_level - 1);
			if (f_vv) {
				cout << "unipoly_domain::power_int "
						"before assign(d,c)" << endl;
			}
			if (f_vv) {
				cout << "b*c=d";
				print_object(d, cout);
	...
	if (f_vv) {
		cout << "unipoly_domain::power_int "
				"before assign(c,a)" << endl;
	}
	assign(c, a, 0 /*verbose_level*/);
	if (f_vv) {
		cout << "unipoly_domain::power_int "
				"before delete_object(b)" << endl;
	}
	delete_object(b);
	delete_object(c);
	delete_object(d);
...
// field of order p (BTW: p might also be a prime power)
{
	...
	create_object_by_rank(u, q + m_alpha, 0 /*verbose_level*/);
	create_object_by_rank(v, 0, 0 /*verbose_level*/);
	create_object_by_rank(w, 0, 0 /*verbose_level*/);
	if (f_vv) {
...
// compute the minimum polynomial of alpha over F_p.
{
	...
	create_object_by_rank_longinteger(b, alpha,
			0 /*verbose_level*/);
...
// Lueneburg~\cite{Lueneburg87a}, p. 112.
{
	...
	for (i = 0; i < deg + 2; i++) {
		if (i == 0) {
			create_object_by_rank(sigma[i], 1, 0 /*verbose_level*/);
		}
		else {
			create_object_by_rank(sigma[i], 0, 0 /*verbose_level*/);
		}
	}
	create_object_by_rank(h, 0, 0 /*verbose_level*/);
	create_object_by_rank(h2, 0, 0 /*verbose_level*/);
	create_object_by_rank(mm, 0, 0 /*verbose_level*/);
	...
	while (true) {
		...
		mult_mod_negated(
				g, sigma[i - 1], sigma[i],
				recalculate_degree(mm), ((int *)mm) + 1,
				verbose_level - 2);
		if (f_vv) {
			cout << "unipoly_domain::minimum_polynomial_extension_field "
					"i = " << i
		...
		for (j = i - 1; j >= 1; j--) {
			...
			mult_mod_negated(
					g, sigma[j - 1], h,
					recalculate_degree(mm), ((int *)mm) + 1, 0);
			if (f_vv) {
				cout << "unipoly_domain::minimum_polynomial_extension_field "
						"i = " << i << " j = "
		...
		for (k = i; k >= 0; k--) {
			if (recalculate_degree(sigma[k]) > 0) {
				break;
			}
		}
		if (k == -1) {
		...
		int verbose_level)
{
	...
	if (f_vv) {
		cout << "unipoly_domain::characteristic_polynomial "
				"before delete_object(M[i * k + j]);" << endl;
	}
	for (i = 0; i < k; i++) {
		for (j = 0; j < k; j++) {
			if (f_vv) {
...
// M is a matrix with polynomial entries
{
	...
	if (k == 0) {
		delete_object(p);
		create_object_by_rank(p, 1, 0 /*verbose_level*/);
		if (f_v) {
	...
	delete_object(p);
	create_object_by_rank(p, 0, 0 /*verbose_level*/);
	create_object_by_rank(p1, 0, 0 /*verbose_level*/);
	create_object_by_rank(p2, 0, 0 /*verbose_level*/);
	create_object_by_rank(p3, 0, 0 /*verbose_level*/);
