		...
		gl_class_rep *R, int verbose_level)
{
	...
	if (f_v) {
		cout << "gl_classes::make_matrix_from_class_rep "
				"before make_matrix_in_rational_normal_form" << endl;
	}
	...
	if (f_v) {
		cout << "gl_classes::make_matrix_from_class_rep "
				"after make_matrix_in_rational_normal_form" << endl;
	}
	FREE_int(Select);
...
loop1:
	...
	while (true) {
		...
		centralizer_order_Kung(
				Select_polynomial, Select_partition, co,
			verbose_level - 2);
		...
		gl_class_rep *R, int *Basis, int verbose_level)
{
	...
	
	{
		...
		if (f_v) {
			cout << "gl_classes::identify_matrix "
					"before factorize_polynomial" << endl;
		}
		...
		if (f_v) {
			cout << "gl_classes::identify_matrix "
					"after factorize_polynomial" << endl;
		}
	...
	int verbose_level)
{
	...
	nb_irreds = other::orbiter_kernel_system::Orbiter->Int_vec->count_number_of_nonzero_entries(
...
// b is the number of columns in Basis before and after
{
	...
	for (f = Data->height; f >= 1; f--) {
		...
		for (e = 0; e < af; e++) {
			...
			Forbidden_subspace = NEW_OBJECT(other::data_structures::int_matrix);
			...
			if (f > 1) {
				...
				other::data_structures::int_matrix *Dummy_subspace;
					
				Dummy_subspace = NEW_OBJECT(other::data_structures::int_matrix);
