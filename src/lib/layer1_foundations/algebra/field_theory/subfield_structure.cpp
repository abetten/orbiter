...
void subfield_structure::lift_vector(
		int *vQ,
...
// output is vq[len * s] over the field Fq,
{
	...
	if (f_v) {
		cout << "subfield_structure::lift_vector" << endl;
	}
	...
	for (i = 0; i < len; i++) {
		...
		Int_vec_copy(components + a * s, vq + i * s, s);
#if 0
		for (j = 0; j < s; j++) {
			b = components[a * s + j];
			vq[i * s + j] = b;
	...
	if (f_v) {
		cout << "subfield_structure::lift_vector done" << endl;
	}
}
