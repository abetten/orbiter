...
// Used in isomorph_BLT
{
	...
	if (go.compare_with_int(10) < 0) {
		...
		for (i = 0; i < go.as_lint(); i++) {
			...
			AAA->Group_element->element_print_as_permutation_with_offset(Elt1, file, 1,
				f_do_it_anyway_even_for_big_degree,
				f_print_cycles_of_length_one,
				0 /* verbose_level */);
			file << "\\\\" << endl;
		...
		for (i = 0; i < go.as_lint(); i++) {
			...
			fp = NEW_int(A->degree);
			n = A->Group_element->find_fixed_points(Elt1, fp, 0);
	...
	if (AAA->Kernel) {
		...
		for (i = 0; i < AAA->Kernel->gens.len; i++) {
			...
			A->Group_element->element_print_latex(AAA->Kernel->gens.ith(i), file);
			file << "$$" << endl;
			//file << "$b_{" << setw(2) << i + 1 << "} = $" << endl;
