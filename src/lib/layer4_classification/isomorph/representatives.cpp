		...
		int verbose_level)
{
	...
	}
	{
		...
		for (i = 0; i < count; i++) {
			...
			for (j = 0; j < SG.len; j++) {
				A->Group_element->element_write_file_fp(SG.ith(j), f2,
						0/* verbose_level*/);
				cnt++;
			}
		}
		...
		for (i = 0; i < count; i++) {
			...
			if (f_vv) {
				...
				for (j = 0; j < len; j++) {
					...
					A->Group_element->print(cout, gens.ith(j));
					cout << endl;
				}
		...
		int verbose_level)
{
	...
	if (f_v) {
		cout << "representatives::get_stabilizer" << endl;
	}
	...
	if (f_v) {
		cout << "representatives::get_stabilizer before "
	...
	if (f_v) {
		cout << "representatives::get_stabilizer done" << endl;
	}
}
