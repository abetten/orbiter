		...
		int verbose_level)
{
	...
	for (cnt = 0; cnt < nb_times; cnt++) {
		...
		Poset->A->Group_element->random_element(S, Elt, 0 /* verbose_level */);
		Poset->A2->Group_element->map_a_set_and_reorder(set1, set2, level, Elt,
				0 /* verbose_level */);
		if (f_v) {
			cout << "mapped set is ";
		...
		Orbit_tracer->recognize(set2, level, transporter, //f_implicit_fusion,
...
// returns next_node
{
	...
	Poset->A2->Group_element->element_retrieve(O->get_E(current_extension)->get_data(), Elt1, 0);
	
	if (f_v) {
		cout << "poset_classification::poset_classification_apply_isomorphism "
				"applying fusion element" << endl;
		Poset->A2->Group_element->element_print_quick(Elt1, cout);
	...
	Poset->A2->Group_element->map_a_set(set_in, set_tmp, size, Elt1, 0);
	if (f_v) {
		cout << "poset_classification::poset_classification_apply_isomorphism "
				"the set becomes: ";
	...
	Poset->A2->Group_element->element_mult(transporter_in, Elt1, Elt2, 0);
	if (f_v) {
		Lint_vec_print(cout, set_in, size);
		cout << endl;
	}
	Poset->A2->Group_element->move(Elt2, transporter_out);
...
// or -1 if f_tolerant and the node could not be found
{
	...
	if (!O->trace_next_point_in_place(
			this,
	...
	if (t == EXTENSION_TYPE_EXTENSION) {
		...
		next_node = poset_classification_apply_isomorphism(
				cur_level, size,
		...
		Poset->A->Group_element->element_move(tmp_Elt1, Elt_transporter, 0);
...
// returns the case number of the canonical set
{
	...
	tmp_Elt1 = NEW_int(Poset->A->elt_size_in_int);
	tmp_Elt2 = NEW_int(Poset->A->elt_size_in_int);
	...
	Poset->A->Group_element->element_one(Elt_transporter, 0);
	...
	if (case_nb < 0) {
		...
		cout << "poset_classification::trace_set, "
				"level = " << level << endl;
		cout << "poset_classification::trace_set, "
				"first_poset_orbit_node_at_level[level] = "
				<< Poo->first_node_at_level(level) << endl;
		exit(1);
	}
	FREE_lint(tmp_set1);
	FREE_lint(tmp_set2);
		...
		long int *set, int len, int verbose_level)
{
	...
	v = NEW_int(Poset->VS->dimension);
	basis = NEW_int(sz * Poset->VS->dimension);
