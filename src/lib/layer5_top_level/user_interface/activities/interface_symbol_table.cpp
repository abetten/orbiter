		...
		int argc, std::string *argv, int &i, int verbose_level)
{
	...
	if (ST.stringcmp(argv[i], "-define") == 0) {
		...
		f_define = true;
		Symbol_definition = NEW_OBJECT(user_interface::core_system::symbol_definition);
		...
		int argc, std::string *argv, int &i, int verbose_level)
{
	...
	while (true) {
		...
		if (ST.stringcmp(argv[i], "-and") == 0) {
			...
			f_activity = true;
			Activity_description = NEW_OBJECT(user_interface::core_system::activity_description);
