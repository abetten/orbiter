...
orthogonal_space_activity_description::orthogonal_space_activity_description()
{
	...
	f_intersect_with_subspace_given_by_base = false;
	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		...
		else if (ST.stringcmp(argv[i], "-intersect_with_subspace_given_by_base") == 0) {
			f_intersect_with_subspace_given_by_base = true;
			intersect_with_subspace_given_by_base_label.assign(argv[++i]);
			if (f_v) {
				cout << "-intersect_with_subspace_given_by_base " << intersect_with_subspace_given_by_base_label << endl;
			}
		}
