...
void bt_key::init_int8(long int field1, long int field2)
{
	init(bt_key_int, 8, field1, field2);
}

void bt_key::init_int4(long int field1, long int field2)
{
	init(bt_key_int, 4, field1, field2);
}

void bt_key::init_int2(long int field1, long int field2)
{
...
int bt_key_int8_cmp(char *p1, char *p2)
{
	int_8 *p_l1, *p_l2;
...
int bt_key_compare(char *key1, char *key2, Vector& V, int depth)
{
	...
	for (i = 0; i < depth; i++) {
		...
		if (k == bt_key_int) {
			if (output_size == 8) {
				res = bt_key_compare_int8(&the_key1, &the_key2);
				if (res) {
					return res;
				}
			}
			...
			for (j = 0; j < Key.int_vec_len(); j++) {
				if (output_size == 8) {
					...
					cout << "bt_key_compare() output_size unrecognized" << endl;
					exit(1);
				}
			}
		}
...
void bt_key_fill_in(char *key, Vector& V, Vector& the_object)
{
	...
	for (i = 0; i < V.s_l(); i++) {
		...
		if (k == bt_key_int) {
			if (output_size == 8) {
				bt_key_fill_in_int8(&the_key, key_object);
			}
			else if (output_size == 4) {
			...
			for (j = 0; j < Key.int_vec_len(); j++) {
				...
				if (output_size == 8) {
					bt_key_fill_in_int8(&the_key, *key_object1);
				}
				else if (output_size == 4) {
