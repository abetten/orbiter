...
// returns next_node
{
	...
	if (f_v) {
		...
		cout << "to the set ";
		lint_vec_print(cout, gen->set[lvl + 1], len + 1);
		cout << endl;
		}
	...
	if (gen->Poset->f_subspace_lattice) {
		...
		lint_vec_copy(set, gen->set[lvl + 1], len + 1);
		}
	else {
		...
		if (f_v) {
			...
			lint_vec_print(cout, gen->set[lvl + 1], lvl + 1);
			cout << endl;
			}
...
// pt0 is the same as E[current_extension].pt !!!
{
	...
	if (f_v) {
		cout << "FUSION NODE at lvl " << lvl
				...
				<< "/" << E[current_extension].data2 /*my_extension*/
				<< " : ";
		lint_vec_print(cout, gen->set0, lvl + 1);
		cout << endl;
#if 0
	...
	if (f_v) {
		...
		lint_vec_print(cout, gen->set1, lvl + 1);
		cout << endl;
		cout << "Elt1=" << endl;
		gen->Poset->A->element_print_quick(gen->Elt1, cout);
	...
	if (f_v) {
		...
		lint_vec_print(cout, gen->set3, lvl + 1);
		cout << endl;
	}
	...
	if (gen->Poset->f_subspace_lattice) {
		...
		Sorting.lint_vec_heapsort(gen->set3, lvl);
		cmp = lint_vec_compare(gen->set3, gen->set0, lvl + 1);
		}
	...
	if (cmp != 0) {
		...
		cout << "comparing ";
		lint_set_print(cout, gen->set3, lvl + 1);
		cout << " with ";
		lint_set_print(cout, gen->set0, lvl + 1);
		cout << endl;
		if (gen->Poset->f_subspace_lattice) {
			int *v;
			int i;
...
// called by poset_orbit_node::trace_next_point
{
	...
	if (Schreier_vector) {
		...
		if (!gen->Schreier_vector_handler->coset_rep_inv_lint(
				Schreier_vector,
				pt_to_trace,
				pt0,
				verbose_level - 1)) {
